...
class AttentionWordRNN(nn.Module):
    ...
    def __init__(self, batch_size, num_tokens, embed_size, word_gru_hidden, bidirectional= True):        
        ...
        self.word_gru_forward= nn.GRU(embed_size, word_gru_hidden)
        self.word_gru_backward= nn.GRU(embed_size, word_gru_hidden)
    ...
    def forward(self, embed, state_word):
        ...
        # word level gru
        state_word_forward, state_word_backward = state_word.chunk(2, dim=0)
        output_word_forward, state_word_forward = self.word_gru_forward(embedded, state_word_forward)
        output_word_backward, state_word_backward = self.word_gru_backward(embedded, state_word_backward)
        output_word = torch.cat([output_word_forward, output_word_backward], dim=0)
        state_word = torch.cat([state_word_forward, state_word_backward], dim=0)
